...
const handleKeyDown = (e, row, column) => {
    ...
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
        ...
        e.preventDefault();
        const Element = document.querySelector(`[data-row-index='${row.index - 1}'][data-column-name='${column.id}']`);
        if(Element){
            Element.focus();
        }
    }
...
export const InputColumnField = ({ row, column }) => {
    return (
        <>
            <div  >
                <Input data-row-index={row.index} data-column-name={column.id} onKeyUp={(e)=> handleKeyUp(e)} onKeyDown={(e) => handleKeyDown(e , row,column)} />
            </div>
        </>
    )
}
...
export const ComboboxColumnField = ({ row, column, cell }) => {
    ...
    const { data, isLoading, isError, refetch } = useFetchSale(new Date(), 'rj41');
